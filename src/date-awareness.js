...
=== END FRESHNESS RULES ===`.trim();
    ...
    return `=== CRITICAL: DATE & FRESHNESS RULES ===
Current real-world date and time (EST): UNKNOWN
You are running LIVE in ${process.env.MODEL_CUTOFF || 'DECEMBER 2024'}. This is NOT a simulation.

YOUR TRAINING DATA ENDS AROUND ${MODEL_CUTOFF.toUpperCase()}. You are MISSING over a year of real-world events, market history, prices, earnings, news, policy changes, and company developments.
...
function ragReminder() {
  try {
    ...
    return `REMINDER: Current date is UNKNOWN. Use ONLY the data provided above. Your training data ends around ${MODEL_CUTOFF} — do NOT use stale internal knowledge for any recent prices, events, or market conditions.`;
  }
}
