...
class InitiativeEngine {
  ...
  async _selfTuneParameters(ctx, tradeHistory) {
    ...
    if (changes.length === 0) {
      this._addJournalEntry('self_tune', `Reviewed ${recent.length} recent trades — no parameter changes needed (WR: ${(winRate * 100).toFixed(0)}%, W:L ${avgWinPct.toFixed(1)}%:${avgLossPct.toFixed(1)}%)`);
      return;
    }
