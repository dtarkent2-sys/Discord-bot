...
const YOLO_BRANCH_PREFIX = 'yolo/improvements'; // daily branch prefix
...
class YoloMode {
  constructor() {
    ...
    // Branch sandboxing: YOLO commits go to a daily branch, not main
    this._currentBranch = this._storage.get('currentBranch', null);
    this._currentPrUrl = this._storage.get('currentPrUrl', null);
  }
...
If you can't think of anything valuable, respond: NO_EVOLUTION`;
  ...
  /**
   * Get or create today's YOLO branch. All YOLO commits land here, not main.
   * Returns branch name on success, null on failure.
   */
  async _ensureYoloBranch() {
    const today = new Date().toISOString().slice(0, 10);
    const branchName = `${YOLO_BRANCH_PREFIX}-${today}`;

    // Reuse if we already set it up today
    if (this._currentBranch === branchName) return branchName;

    const created = await github.ensureBranch(branchName);
    if (!created) {
      console.error(`[YOLO] Failed to create branch ${branchName}`);
      return null;
    }

    this._currentBranch = branchName;
    this._currentPrUrl = null; // reset PR for new branch
    this._storage.set('currentBranch', branchName);
    this._storage.set('currentPrUrl', null);
    return branchName;
  }

  /**
   * Ensure a PR exists for the current YOLO branch → main.
   */
  async _ensureYoloPR(branchName) {
    if (this._currentPrUrl) return this._currentPrUrl;

    const today = new Date().toISOString().slice(0, 10);
    const pr = await github.ensurePullRequest(
      branchName,
      `YOLO improvements ${today}`,
      `Automated improvements from Billy's YOLO mode on ${today}.\n\n` +
      `These changes were generated by Billy's self-improvement engine and need review before merging to main.\n\n` +
      `Use \`/yolo history\` in Discord to see what was changed.`
    );

    if (pr) {
      this._currentPrUrl = pr.url;
      this._storage.set('currentPrUrl', pr.url);
      if (pr.created) {
        this._post(
          `**YOLO Branch Created**\n` +
          `Today's improvements will go to branch \`${branchName}\`\n` +
          `PR: ${pr.url}\n` +
          `_Review & merge when you're ready — main stays safe._`
        );
      }
      return pr.url;
    }
  ...
  async _generateAndApplyFix(filePath, instruction, source) {
    // Ensure we have a YOLO branch — never commit to main
    const branch = await this._ensureYoloBranch();
    if (!branch) {
      this._addJournal('blocked', filePath, 'Failed to create YOLO branch', source);
      return null;
    }

    // Fetch current code from main (source of truth)
...
${currentCode}`;
    ...
    // Commit to the YOLO branch (not main)
    const commitMsg = `YOLO: ${instruction.slice(0, 70)}`;
    const result = await github.updateFileOnBranch(filePath, newCode, commitMsg, branch);
    ...
    if (result.success) {
      ...
      const prUrl = await this._ensureYoloPR(branch);
      ...
      this._addJournal('improvement', filePath, `${instruction.slice(0, 150)} (${linesChanged} lines) [branch: ${branch}]`, source);
      auditLog.log('yolo', `Improvement applied: ${filePath} (${linesChanged} lines, source: ${source}, branch: ${branch})`);
      ...
      this._post(
        ...
        `**Branch:** \`${branch}\`${prUrl ? ` — [Review PR](${prUrl})` : ''}\n` +
        `${result.url ? result.url : ''}\n` +
        `_Improvements today: ${this._dailyCount}/${MAX_IMPROVEMENTS_PER_DAY} — main is protected_`
      );
  ...
  getStatus() {
    ...
    return {
      ...
      branch: this._currentBranch || 'none (will create on next commit)',
      prUrl: this._currentPrUrl || null,
      mainProtected: true,
    };
  }
