...
class ValideaService {
  ...
  async _login() {
    ...
    let getRes;
    try {
      const getResTmp = await fetch(`${VALIDEA_HOST}/login`, {
        headers: BROWSER_HEADERS,
        redirect: 'follow',
        signal: AbortSignal.timeout(15000),
      });
      const getCookies = getResTmp.headers.getSetCookie ? getResTmp.headers.getSetCookie() : [];
      initialCookies = this._parseCookies(getCookies.length > 0 ? getCookies : [getResTmp.headers.get('set-cookie') || '']);
      console.log(`[Validea] Login page: status ${getResTmp.status}, cookies: ${getCookies.length}`);

      // Try to find the login form action from the HTML
      if (getResTmp.ok) {
        const html = await getResTmp.text();
