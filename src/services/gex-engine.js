class GEXEngine {
  ...
  async analyze(ticker, opts = {}) {
    ...
    log.debug(`Walls: callWall=${pods callWalls[0] ? '$' + pods callWalls[0].strike : 'none'}, putWall=${pods putWalls[0] ? '$' + pods putWalls[0].strike : 'none'}, flip=${gammaFlip ? '$' + gammaFlip : 'none'}`);
   ...
   *   adjusted by distance from flip (if known)
   *   Capped at 1.0.
   */
  ...
  _generatePlaybook(regime, walls, gammaFlip, spot) {
    ...
    if (regime.label === 'Long Gamma') {
      ...
      if (primaryPut && spot < primaryPut.strike) {
        const target = nextPut ? `$${Math.abs(primaryPut.strike)}` : 'next support';
